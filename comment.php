...
function wp_trash_comment($comment_id) {
	...
	if ( wp_set_comment_status($comment_id, 'trash') ) {
		...
		$table_name = "my_comments_.$user_id";


		//$query= "DELETE FROM `wordpress`.`my_comments_3` WHERE `my_comments_3`.`author_comment_ID` = 6";

		//$query= 'DELETE FROM `wordpress`.'."$table_name".' WHERE `my_comments_3`.`author_comment_ID` = '."$comment_id";

	$query= "DELETE FROM my_comments_$user_id WHERE my_comments_$user_id.`author_comment_ID` = $comment_id";

	//	$query= "DELETE FROM '"."$wpdb.$table_name"."' WHERE '"."$table_name".'author_comment_ID'."="."$comment_id"";

		$wpdb->query($query);


		//$author_comment_ID = $comment->comment_ID;

		//if(!$wpdb->delete($table_name, array('user_id'=>"$user_id", 'author_comment_ID'=>"$comment_id"))){
		//	echo 'did not delete';
		//}
...
$post_author = $wpdb->get_var($wpdb->prepare("SELECT post_author FROM $wpdb->posts WHERE ID = %d LIMIT 1", $comment_post_ID));
	...
	if($wpdb->insert('my_comments_'.$user_id,array('author_comment_ID'=>$id,'post_author_ID'=>$post_author,'comment_post_ID'=>$comment_post_ID,'comment_date'=>$comment_date,'comment_content'=>$comment_content,'user_id'=>$user_id))){
